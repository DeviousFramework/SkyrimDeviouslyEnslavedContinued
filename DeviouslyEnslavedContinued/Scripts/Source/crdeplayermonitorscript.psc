...
dfwDeviousFramework _qDeviousFramework
...
bool                               masterIsSlaver    ; what was I going to do with this again?
...
Float Property  fLastEnslaveAttempt     = 0.0 Auto 
...
Event OnUpdate()
  ...
  ; If the script has been upgrade handle that now.
  float fScriptVersion = 0.01
  if (fScriptVersion > _fCurrVer)
    UpgradeScript(fScriptVersion)
    ; Delay for a period before continuing with the poll... Let things initialize
    RegisterForSingleUpdate(5)
    return
  endif

  if(MCM.gCRDEEnable.GetValueInt() == 0) ; mod is off
    debugmsg("Mod Disabled.", 1)
    ...
    if player.IsInCombat()
      ;debugmsg("Update Outcome: In Combat", 1)
    ...
    elseif wasInCombat == true ;&& not in combat, but we can't get here if this is false anyway
      ;debugmsg("Update Outcome: Was In Combat", 1)
      wasInCombat = false
      ...
      ; If the thread is busy decrease the wait count.  (Do this regardless of vulnerability)
      if (0 < iThreadBusy)
        debugmsg("Mod Still Busy.  Waiting: " + iThreadBusy, 3)
        iThreadBusy -= 1
      endif

      ; Keep track of how long we want to wait for the next update interval.
      ; The actual register is performed after determining what activity we are doing.
      float fNextUpdate = MCM.fEventInterval

      ; Only consider more options if the player is vulnerable enough for sex or enslavement.
      ; Also don't consider options if neither sex nor enslavement is allowed.
      bool bTryApproach = true
      if (_qDeviousFramework && MCM.bPreferDfw)
        int iDfwVulnerability = (_qDeviousFramework.GetVulnerability() * MCM.iPercentDfwVulnerability / 100)
        if (!_qDeviousFramework.IsAllowed(_qDeviousFramework.AP_SEX + _qDeviousFramework.AP_ENSLAVE))
          debugmsg("Sex and enslavement is not allowed.", 3)
          bTryApproach = false
        elseif ((iDfwVulnerability < MCM.iMinVulnerabilitySex) && (iDfwVulnerability < MCM.iMinVulnerabilityEnslave))
          ; Can't show less than symbol here.  Limitation of Debug.Notification().
          debugmsg("Player not Vulnerable: " + iDfwVulnerability + " less than (" + MCM.iMinVulnerabilitySex + \
                   " and " + MCM.iMinVulnerabilityEnslave + ")", 3)
          bTryApproach = false
        endIf
      endIf
      ...
      if (bTryApproach)
        ...
        ;Mods.isPlayerInJail()
        if forceGreetIncomplete
          fNextUpdate = 5  ; faster because we want to catch conditions, for now static
        endif
      ...
      ; Now we know how long we should wait, register for an update.
      RegisterForSingleUpdate(fNextUpdate)
    endif

    ; We can assume this doesn't change while the game is loaded.  We should probably move this
    ; to an OnPlayerLoadGame() event instead (Note: A player alias must be seutp for that).
  ...
  debugmsg("OnUpdate time spent:" + (Utility.GetCurrentRealTime() - fStartTime))
EndEvent
...
function UpgradeScript(float fNewVersion)
  debugmsg("Updating Script " + _fCurrVer + " => " + fNewVersion, 4)

  ; Very basic initialization.
  if (0.01 > _fCurrVer)
    ; TODO: All quest references should be set on each game load as various mod changes can
    ; cause quest reference variables to become invalid.
    _qDeviousFramework = Quest.GetQuest("_dfwDeviousFramework") As dfwDeviousFramework
  endIf

  ; Finally update the version number.
  _fCurrVer = fNewVersion
endfunction
...
bool function attemptEnslavement(actor masterRef, bool skipMsg = false)
  ...
  if (attackerRefAlias.GetActorReference() != player && \
      masterRef.isDead() == false && \
      masterRef.isChild() == false && \
      masterRef.isPlayerTeammate() == false && \
      masterRef.isAIEnabled() == true); && masterRef.getRace().isPlayable() == true)
    ...
    if (masterRef.getDistance(player) > 500)
      player.MoveTo(masterRef) 
      debugmsg(masterRef.getDisplayName() + " grabs you as you try to run.", 5)
    endIf

    fLastEnslaveAttempt = Utility.GetCurrentGameTime()
    if (skipMsg == false)
    ...
    if  roll <= MCM.iEnslaveWeightSD
      if (!_qDeviousFramework || (0 <= _qDeviousFramework.SetMaster(None, "", _qDeviousFramework.AP_ALL, _qDeviousFramework.MD_CLOSE, True)))
        debugmsg("Starting SD", 1)
        Mods.clearHelpless()
        Mods.enslaveSD(masterRef)
        return true
      endif
      debugmsg("attemptEnslavement err: could not overthrow current DFW master", 5)
...
bool function tryEnslavableSexEnd(actor actorRef)
  ...
  if (rollEnslave != 0 && rollEnslave < MCM.iRapeEventEnslave )
    if (!_qDeviousFramework || _qDeviousFramework.IsAllowed(_qDeviousFramework.AP_ENSLAVE))
      ...
      debugmsg("Enslaving player not allowed.", 2)
    endif
  endif

  if(rollDevice < MCM.iRapeEventDevice && enslavedLevel == 0)
    if (!_qDeviousFramework || _qDeviousFramework.IsAllowed(_qDeviousFramework.AP_RESTRAIN))
      ...
      debugmsg("Restraining player not allowed.", 2)
    endif
  endif
...
bool function attemptApproach()
  ...
  if isPlayerBusy ; lets test this sooner, fewer moot cycles
    ;debugmsg("Update Outcome: Player Busy", 1)
    ...
    clear_force_variables(true) ; <- added 11.2.3
  ...
  ; If there is no nearest actor see if we can get one from the Devious Framework.
  if ((None == nearest[0]) && _qDeviousFramework)
    actor aDfwNearest = _qDeviousFramework.GetRandomActor(iIncludeFlags=_qDeviousFramework.AF_DOMINANT, \
                                                          iExcludeFlags=(_qDeviousFramework.AF_GUARDS + _qDeviousFramework.AF_MERCHANTS))
    if (aDfwNearest)
      nearest = new actor[1]
      nearest[0] = aDfwNearest
    endif
  endif
  ...
  if  MCM.bFollowerDialogueToggle.GetValueInt() == 1 \
    ...
    while i < followers.length
      ...
      elseif tmp_follower != None && !tmp_follower.WornHasKeyword(libs.zad_DeviousGag) && !tmp_follower.WornHasKeyword(libs.zad_DeviousArmbinder)
        ...
        if SexLab.HadPlayerSex(tmp_follower) || StorageUtil.GetFloatValue(tmp_follower, "crdeThinksPCEnjoysSub") > 0 || valid_count == 0
  ...
  ; Keep track of the Devious Framework (DFW) vulnerability if the mod is installed.
  int iDfwVulnerability = 0
  if _qDeviousFramework
    iDfwVulnerability = (_qDeviousFramework.GetVulnerability() * MCM.iPercentDfwVulnerability / 100)
  endIf
  string szDebugMessage = "Vulerability: " + iDfwVulnerability

  ; First try for enslavement.
  bool bEnslave
  string szInfoMessage
  if (!_qDeviousFramework || ((iDfwVulnerability >= MCM.iMinVulnerabilityEnslave) && \
                               _qDeviousFramework.IsAllowed(_qDeviousFramework.AP_ENSLAVE)))
    float fRoll = Utility.RandomFloat(0, 100)
    float fModified = fRoll / rollModifier

    if isSlaver
      fModified /= MCM.fModifierSlaverChances
    endIf

    ; DFW vulnerability accounts for chastity.  If it's not installed account for it here.
    if (!_qDeviousFramework && MCM.bChastityToggle)
      ...
      if wearingBlockingFull
        fRoll = fRoll / MCM.fChastityCompleteModifier
      elseIf wearingBlockingAnal || wearingBlockingVaginal || wearingBlockingBra
        fRoll = fRoll / MCM.fChastityPartialModifier
      endIf
    endIf

    ; Figure out the maximum roll to trigger an enslavement conversation.
    float fMaxRoll = MCM.iChanceEnslavementConvo

    ; Adjust the maximum based on current vulnerability.
    if (_qDeviousFramework)
      fMaxRoll += (iDfwVulnerability * MCM.iPercentVulnerabilityEnslave / 100)
    endIf

    ; Can't show less than symbol here.  Limitation of Debug.Notification().
    szInfoMessage = "Enslave(" + fModified + " {= " + fMaxRoll + ")?"
    szDebugMessage += " Enslave Base: " + fRoll + " / " + MCM.iChanceEnslavementConvo
    if (fModified <= fMaxRoll)
      bEnslave = True
    endIf
  endIf
  ...
  ; Then try for sex.
  Bool bDoSex
  if (!bEnslave && (!_qDeviousFramework || (iDfwVulnerability >= MCM.iMinVulnerabilitySex) && \
                                            _qDeviousFramework.IsAllowed(_qDeviousFramework.AP_SEX)))
    float fRoll = Utility.RandomFloat(0, 100)
    float fModified = fRoll / rollModifier
    ...
    ; Figure out the maximum roll to trigger an enslavement conversation.
    float fMaxRoll = MCM.iChanceSexConvo

    ; Adjust the maximum based on current vulnerability.
    if (_qDeviousFramework)
      fMaxRoll += (iDfwVulnerability * MCM.iPercentVulnerabilitySex / 100)
    endIf

    ; DFW vulnerability accounts for chastity.  If it is not installed account for it here.
    if (!_qDeviousFramework && MCM.bChastityToggle)
      debugmsg("chastity:" + wearingBlockingFull + ": a:" + wearingBlockingAnal + " b:" + wearingBlockingBra + " g:" + wearingBlockingGag, 3)
      if wearingBlockingFull
        ; all items
        ...
        || (nearest[0].getItemCount(libs.restraintsKey) > 0 && !libs.GetWornDeviceFuzzyMatch(player, libs.zad_DeviousGag).HasKeyword(libs.zad_BlockGeneric))
          ...
          fMaxRoll = 0.0  ; impossible, attacker has no keys.
        endIf
      elseIf wearingBlockingAnal || wearingBlockingVaginal || wearingBlockingBra
        fRoll = fRoll / MCM.fChastityPartialModifier
      endIf
    ...
    If (_qDeviousFramework.GetMaster(_qDeviousFramework.MD_CLOSE))
       fMaxRoll *= (MCM.iPercentSexWhenEnslaved As float) / 100.0
    EndIf

    ; Can't show less than symbol here.  Limitation of Debug.Notification().
    szInfoMessage += "Sex(" + fModified + " {= " + fMaxRoll + ")?"
    szDebugMessage += " Sex Base: " + fRoll + " / " + MCM.iChanceSexConvo
    if (fModified <= fMaxRoll)
      bDoSex = True
    endIf
  EndIf

  debugmsg(szInfoMessage, 3)
  debugmsg(szDebugMessage, 3)

  ; Make sure the player is not chastity protected (this is handled above if DFW is not installed).
  if (_qDeviousFramework && bDoSex && MCM.bChastityToggle && wearingBlockingFull)
    debugmsg("Chastity: Anal(" + wearingBlockingAnal + ") Bra(" + wearingBlockingBra + ") Gag(" + wearingBlockingGag + ")", 3)

    ; The belt is only a problem if the actor has no keys or it has a generic block.
    Int iChastityKeyCount = nearest[0].GetItemCount(libs.chastityKey)
    Bool bChastityBlocked = libs.GetWornDeviceFuzzyMatch(player, libs.zad_DeviousBelt).HasKeyword(libs.zad_BlockGeneric)
    if (!iChastityKeyCount || bChastityBlocked)
      ; If the belt and gag are both inaccessible sex is not possible.
      Int iRestraintKeyCount = nearest[0].getItemCount(libs.restraintsKey)
      Bool bGagBlocked = libs.GetWornDeviceFuzzyMatch(player, libs.zad_DeviousGag).HasKeyword(libs.zad_BlockGeneric)
      if (!iRestraintKeyCount || bGagBlocked)
        debugmsg("Player protected: Chastity Blocked.", 3)
        bDoSex = False
      endIf
    endIf
  endIf
  ...
  if (!bEnslave && !bDoSex)
    debugmsg("Stopping: Rolled too high.", 3)
    return false
  endIf
  ...
  ; If the player is not vulnerable but DFW indicates she is, make an adjustment.
  ; For example, this could happen if she is locked in BDSM furniture.
  if (!vulnerability && MCM.bPreferDfw)
    ; Just our getting here is an indication of DFW vulnerability so make the adjustment.
    ; For now let's say a DFW vulnerability 10-35 = 1... 35-60 = 2... 60-85 = 3...  85+ = 4.
    vulnerability = ((iDfwVulnerability + 15) / 25)
  endIf
  ...
  if enslavedLevel != 3 && (vulnerability > 0 || enslavedLevel == 1) && \
    ...
    ; enslave attempt
    if (bEnslave) ; some of this should have been detected earlier
      if !Mods.hasAnySlaveMods
        debugmsg("No slave mods, cannot enslave", 3)
      elseif (CurrentGameTime < (fLastEnslaveAttempt + MCM.fEventTimeout))
        debugmsg("Not enough time has passed since the last enslave attempt: now:" + CurrentGameTime + " t.o.:" + (fLastEnslaveAttempt + MCM.fEventTimeout), 3)
      ...
      ; If using DFW checking for vulnerability is done elsewhere.
      elseif (!MCM.bPreferDfw || !_qDeviousFramework) && vulnerability < MCM.iMinEnslaveVulnerable
    ...
    ; sex attempt
    ;if(vulnerability > 0   && CurrentGameTime >= (fLastEnslaveAttempt + MCM.fEventTimeout)) && \
...
function setMaster(actor masterRef)
  ...
  masterIsSlaver = Mods.isSlaveTrader(master); was this ever used?
                                             ; No.  Never used outside this file since it is not a Property.
endFunction
...
; this was going to be used back when users complained that followers got in the way of enslavement, instead they complained to the slave mod authors ¯\_(?)_/¯
...
function doPlayerSex(actor actorRef, bool rape = false, bool soft = false, bool oral_only = false)
  ...
  if (playerGender == 1 && actorGender == 1 )
    ...
    ;  why does sorting the actors do this? no fucking clue. this code was used in petcollar, maybe it's placebo who knows ¯\_(?)_/¯
